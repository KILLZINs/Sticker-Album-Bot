...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  try {
    const [emojis, minhaFigCatalogoResult, deleFigCatalogoResult] = await Promise.all([
      getGuildEmojis(guildId),
      db
        .select()
        .from(catalogoFigurinhasTable)
        .where(and(eq(catalogoFigurinhasTable.guildId, guildId), eq(catalogoFigurinhasTable.numero, meuNumero)))
        .limit(1),
      db
        .select()
        .from(catalogoFigurinhasTable)
        .where(and(eq(catalogoFigurinhasTable.guildId, guildId), eq(catalogoFigurinhasTable.numero, delesNumero)))
        .limit(1),
    ]);

    const minhaFigCatalogo = minhaFigCatalogoResult[0];
    const deleFigCatalogo = deleFigCatalogoResult[0];
    ...
    const [minhaNaColecao] = await db
      ...
      .where(and(eq(colecaoUsuarioTable.guildId, guildId), eq(colecaoUsuarioTable.userId, meuId), eq(colecaoUsuarioTable.catalogoId, minhaFigCatalogo.id)))
      .limit(1);

    if (!minhaNaColecao) {
      await interaction.editReply(`❌ Você não tem a figurinha **#${meuNumero} — ${minhaFigCatalogo.titulo}** no seu álbum!`);
      return;
    }
    ...
    const [deleNaColecao] = await db
      .select()
      .from(colecaoUsuarioTable)
      .where(and(eq(colecaoUsuarioTable.guildId, guildId), eq(colecaoUsuarioTable.userId, destino.id), eq(colecaoUsuarioTable.catalogoId, deleFigCatalogo.id)))
      .limit(1);

    if (!deleNaColecao) {
      await interaction.editReply(`❌ <@${destino.id}> não tem a figurinha **#${delesNumero} — ${deleFigCatalogo.titulo}** no álbum!`);
      return;
    }
    ...
    const [jaTemDelesNaMinha] = await db
      .select()
      .from(colecaoUsuarioTable)
      .where(and(eq(colecaoUsuarioTable.guildId, guildId), eq(colecaoUsuarioTable.userId, meuId), eq(colecaoUsuarioTable.catalogoId, deleFigCatalogo.id)))
      .limit(1);

    if (jaTemDelesNaMinha) {
      await interaction.editReply(`⚠️ Você já tem a figurinha **#${delesNumero} — ${deleFigCatalogo.titulo}** no seu álbum!`);
      return;
    }
    ...
    const [deleJaTemMinha] = await db
      .select()
      .from(colecaoUsuarioTable)
      .where(and(eq(colecaoUsuarioTable.guildId, guildId), eq(colecaoUsuarioTable.userId, destino.id), eq(colecaoUsuarioTable.catalogoId, minhaFigCatalogo.id)))
      .limit(1);

    if (deleJaTemMinha) {
      await interaction.editReply(`⚠️ <@${destino.id}> já tem a figurinha **#${meuNumero} — ${minhaFigCatalogo.titulo}** e não vai querer receber em troca!`);
      return;
    }

    const emojiMinha = getRaridadeEmoji(emojis, minhaFigCatalogo.raridade);
    const emojiDele = getRaridadeEmoji(emojis, deleFigCatalogo.raridade);
    ...
    const embedProposta = new EmbedBuilder()
      ...
      .setColor(0x7B2FBE)
      .addFields(
        { name: `${emojiMinha} Dá`, value: `[#${minhaFigCatalogo.numero}] ${minhaFigCatalogo.titulo}`, inline: true },
        { name: "↔️", value: "troca por", inline: true },
        { name: `${emojiDele} Recebe`, value: `[#${deleFigCatalogo.numero}] ${deleFigCatalogo.titulo}`, inline: true }
      )
      .setFooter({ text: `⏳ <@${destino.id}> tem 2 minutos para aceitar ou recusar.` })
      .setTimestamp();

    const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder().setCustomId("aceitar_troca").setLabel("✅ Aceitar troca").setStyle(ButtonStyle.Success),
      new ButtonBuilder().setCustomId("recusar_troca").setLabel("❌ Recusar").setStyle(ButtonStyle.Danger)
    );
    ...
    collector.on("collect", async (btnInteraction: ButtonInteraction) => {
      ...
      try {
        ...
        await db.delete(colecaoUsuarioTable).where(eq(colecaoUsuarioTable.id, minhaAtual.id));
        await db.delete(colecaoUsuarioTable).where(eq(colecaoUsuarioTable.id, deleAtual.id));

        await db.insert(colecaoUsuarioTable).values({ guildId, userId: meuId, username: interaction.user.username, catalogoId: deleFigCatalogo.id });
        await db.insert(colecaoUsuarioTable).values({ guildId, userId: destino.id, username: destino.username, catalogoId: minhaFigCatalogo.id });
        ...
        await btnInteraction.update({ content: "❌ Erro ao realizar a troca. Tente novamente.", embeds: [], components: [] });
      }
    });
