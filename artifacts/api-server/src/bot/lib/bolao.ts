...
export async function gerarEmbedBolao(bolaoId: number) {
  ...
  if (bolao.tipo === "normal" && bolao.premio) {
    ...
    // Prêmio acumulativo: calcular total se adicional for numérico
    let premioAcumulativoLabel = "Acumulativo";
    if (bolao.adicional) {
      const numAdicional = parseFloat(bolao.adicional.replace(",", "."));
      if (!isNaN(numAdicional) && palpites.length > 0) {
        const total = numAdicional * palpites.length;
        // Tentar manter o prefixo/sufixo original (ex: "R$ 10" → "R$ 30")
        const prefixMatch = bolao.adicional.match(/^([^0-9,.\-]*)[\d,.]+([^0-9,.]*)$/);
        const prefix = prefixMatch ? prefixMatch[1] : "";
        const suffix = prefixMatch ? prefixMatch[2] : "";
        premioAcumulativoLabel = `${prefix}${total}${suffix} _(${bolao.adicional} × ${palpites.length})_`;
      } else if (!isNaN(numAdicional) && palpites.length === 0) {
        premioAcumulativoLabel = `${bolao.adicional} por participante`;
      } else {
        premioAcumulativoLabel = `${bolao.adicional} × ${palpites.length} participantes`;
      }
    }
    embed.addFields(
      { name: "📈 Prêmio acumulado", value: `\`${premioAcumulativoLabel}\``, inline: true },
      {
        name: "💸 Aposta mínima",
        value: semValor ? "`Livre`" : `\`${bolao.valorMinimo}\``,
        inline: true,
